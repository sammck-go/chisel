...
func(d ChannelEndpointDescriptor) Validate() error {
  if d.Role != ChannelEndpointRoleStub && d.Role != ChannelEndpointRoleSkeleton {
      return errors.New(fmt.Sprintf("%s: Unknown role type '%s'", d.String(), d.Role))
  }
  if d.Type == ChannelEndpointTypeTCP {
    if d.Path == "" {
      if d.Role == ChannelEndpointRoleStub {
        return errors.New(fmt.Sprintf("%s: TCP stub endpoint requires a bind address and port", d.String()))
      } else {
        return errors.New(fmt.Sprintf("%s: TCP skeleton endpoint requires a target hostname and port", d.String()))
      }
    } else {
      _, port, err := ParseHostPort(d.Path, InvalidPort)
      if err != nil {
        if d.Role == ChannelEndpointRoleStub {
          return errors.New(fmt.Sprintf("%s: TCP stub endpoint <bind-address>:<port> is invalid: %v", d.String(), err))
        } else {
          return errors.New(fmt.Sprintf("%s: TCP skeleton endpoint <hostname>:<port> is invalied: %v", d.String(), err))
        }
      }
      if port == InvalidPort {
        return errors.New(fmt.Sprintf("%s: TCP endpoint requires a port number: %s", d.String()))
      }
    }
  } else if d.Type == ChannelEndpointTypeUnix {
    if d.Path == "" {
      return errors.New(fmt.Sprintf("%s: Unix domain socket endpoint requires a socket pathname", d.String()))
    }
  } else if d.Type == ChannelEndpointTypeLoop {
    if d.Path == "" {
      return errors.New(fmt.Sprintf("%s: Loop endpoint requires a loop name", d.String()))
    }
  } else if d.Type == ChannelEndpointTypeStdio {
    if d.Path != "" {
      return errors.New(fmt.Sprintf("%s: STDIO endpoint cannot have a path", d.String()))
    }
  } else if d.Type == ChannelEndpointTypeSocks {
    if d.Path != "" {
      return errors.New(fmt.Sprintf("%s: SOCKS endpoint cannot have a path", d.String()))
    }
    if d.Role != ChannelEndpointRoleSkeleton {
      return errors.New(fmt.Sprintf("%s: SOCKS endpoint must be placed on the skeleton side", d.String()))
    }
  } else {
    return errors.New(fmt.Sprintf("%s: Unknown endpoint type '%s'", d.String(), d.Type))
  }
  return nil
}
...
func(d ChannelEndpointDescriptor) LongString() string {
  ...
  return "ChannelEndpointDescriptor(role='" + roleName + "', resource='" + d.String() + "')"
}
...
func(d ChannelDescriptor) Validate() error {
  ...
  if d.Stub.Role != ChannelEndpointRoleStub {
    return errors.New(fmt.Sprintf("%s: Role of stub must be ChannelEndpointRoleStub", d.String()))
  }
  if d.Skeleton.Role != ChannelEndpointRoleSkeleton {
    return errors.New(fmt.Sprintf("%s: Role of skeleton must be ChannelEndpointRoleSkeleton", d.String()))
  }

  if (!d.Reverse && d.Skeleton.Type == ChannelEndpointTypeStdio) || 
     (d.Reverse && d.Stub.Type == ChannelEndpointTypeStdio) {
      return errors.New(fmt.Sprintf("%s: STDIO endpoint must be on client proxy side", d.String()))
  }
...
func SplitChannelDescriptorParts(s string) ([]string, error) {
  ...
  flushPartial := func(final bool) error {
    if ! bStack.isBalanced() {
        return errors.New(fmt.Sprintf("SplitChannelDescriptorParts: unmatched '%c' in descriptor '%s'", bStack.popBracket(), s))
    }
    if haveBackslash {
      return errors.New(fmt.Sprintf("SplitChannelDescriptorParts: descriptor ends in backslash: '%s'", s))
    }
  ...
  for _, c := range s {
    ...
    if haveBackslash {
      ...
      if bStack.isBalanced() {
        return nil, errors.New(fmt.Sprintf("SplitChannelDescriptorParts: unmatched '%c' in descriptor '%s'", c, s))
      }
...
func ParseChannelDescriptor(s string) (*ChannelDescriptor, error) {
  ...
  if len(parts) <= 0 {
    return nil, errors.New(fmt.Sprintf("Empty channel descriptor string not allowed: '%s'", s))
  }
  ...
  flushPartialEndpoint := func() error {
    if haveType {
      if haveSkeleton {
        return errors.New(fmt.Sprintf("Too many elements in channel descriptor string: '%s'", s))
      }
      if !haveStub {
  ...
  if d.Stub.Type == ChannelEndpointTypeUnknown {
    return nil, errors.New(fmt.Sprintf("Insufficient information in channel descriptor string: '%s'",s))
  }
  ...
  if d.Stub.Type == ChannelEndpointTypeSocks {
    return nil, errors.New(fmt.Sprintf("SOCKS endpoints are only allowed on the skeleton side: '%s'", s))
  }
  ...
  if d.Skeleton.Type == ChannelEndpointTypeTCP {
    if len(d.Skeleton.Path) > 0 {
      skeletonHost, skeletonPort, err = ParseHostPort(d.Stub.Path, InvalidPort)
      if err != nil {
        return nil, err
      }
    }
  ...
  if d.Stub.Type == ChannelEndpointTypeTCP {
    if stubBindAddr == "" {
      return nil, errors.New(fmt.Sprintf("Unable to determine stub bind address in channel descriptor string: '%s'", s))
    }
    if stubPort == InvalidPort {
      return nil, errors.New(fmt.Sprintf("Unable to determine stub port number in channel descriptor string: '%s'", s))
    }
  ...
  if d.Skeleton.Type == ChannelEndpointTypeTCP {
    if skeletonHost == "" {
      return nil, errors.New(fmt.Sprintf("Unable to determine skeleton host name in channel descriptor string: '%s'", s))
    }
    if skeletonPort == InvalidPort {
      return nil, errors.New(fmt.Sprintf("Unable to determine skeleton port number in channel descriptor string: '%s'", s))
    }
  ...
  if (d.Stub.Type == ChannelEndpointTypeStdio && d.Reverse) || (d.Stub.Type == ChannelEndpointTypeStdio && d.Reverse) {
    return nil, errors.New(fmt.Sprintf("Stdio endpoints are only allowed on the client proxy side: '%s'", s))
  }

  if d.Skeleton.Type == ChannelEndpointTypeUnknown {
    return nil, errors.New(fmt.Sprintf("Unable to determine skeleton endpoint type: '%s'", s))
  }
